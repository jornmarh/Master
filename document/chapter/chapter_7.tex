...
From table 8.3 we find that SQSs B D and E all display band gaps around 0.3 eV in spin up, and small non-zero or zero in the case of SQS D in spin down. To further explore the observed band gaps we plot the projected density of states of SQS D. With this information we can study the above density of states by the contributions of individual elements in the structure. In the PDOS plot we assume that the lines of TMs is mainly attributed to the 3d electrons of the elements. Likewise we state that the blue line corresponding to silicon is composed of s electrons in the lower energy region and and p electrons at higher energies. Ideally this should be  investigating with the local density of states of each element, but with 5 different elements this become tedious. Furthermore this is seen in several transition-metal silicides \textbf{cite?}
...
From figure 8.3 we note that the low energy states states are filled by silicon s electrons. At higher energies there is evidence of hybridization between silicon p and TM 3d electrons. In both spin channels Ni lie at the lowest energies of the 3d elements, followed by iron and then manganese and chromium very close to the fermi energy. Above the fermi enery we see than the spin up channel is equally distributed by all elements while the down states are more dominated by manganese and chromuim. Bellow we include the same figure, but now emphasizing states close to the Fermi energy.
...
Comparing to the equivalent case in SQS B in figure 8.5 we see that the spin down channel in D contain a more dominant presence of manganese especially, and some chromium. 
