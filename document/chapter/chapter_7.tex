...
or formation energy per atom $E_{FPA} = 0.441 eV$ from $-21.16/48$. The total energy of iron and silicon was calculated separately for the respective base elements with identical parameters as used for the \ch{FeSi2} calculation. The total energies correspond well with the listed energies from materials project of -5.4234 eV and -8.4693 eV for Fe and Si. Accordingly the formation energy per atom for $\beta-$ \ch{FeSi2} of 0.441 eV is in good agreement with materials project's value of 0.444 eV for $\beta-$. Again, the difference is most likely related to materials project utilizing a larger energy cutoff of 520 eV compared to our value of 300 eV.
...
Compared to the bulk material we find that the alloy as apparent in all five supercells display a finite magnetic moment of around $0.083 \mu_B$. The local magnetic moments vary slighlty depending on the structure, but is overall largest around chromium atoms in the lattice, closely followed by manganese. Contrary, the local magnetic moments of iron atoms in the lattice is most frequently slightly negative with instances of nonmagnetic or slightly positive moment in certain supercells. Lastly, nickel atoms display small negative local magnetic moments across all structures. 
It should be noted however that the magnetic properties in this project are prone to errors, for instance one of the major drawbacks of DFT in regards to magnetic materials is the local minima problem. In this project we have overlooked this concern and calculated each structure under the same initial magnetic configuration (co-linear) in order to reduce the workload. Coupling this with the possible errors associated with the special quasi-random structures method to model the disordered magnetic structure means that the magnetic results and following the total energy and corresponding stability are not immutable nor necessarily accurate in respect to the hypothetical real alloy. Considering that the total magnetic moment is identical in 5 distinct atomic configurations and the local magnetic moments show very similar trends, the observed magnetism within the scope of this project is most probably connected to the particular crystal structure. We will elaborate on this point when we are discussing the pair distribution functions in section 7.4. 
    
In terms of the total energy the most and least stable SQSs are "D" and "A" respectively. Based on the total energy then, SQS D is then the most representative configuration of the real material. However most likely all five SQSs and other possible SQSs would appear as local orderings in domains of the real material with a certain probability. Therfore we will consider the results of the other SQSs as well as the most stable supercell. Furthermore the total energy alone is not sufficient to evaluate the stability. In this project we have not considered factors such as the confirational entropy or the optimized magnetic configuration as mentioned above. Additionally all calculations and corresponding results are only valid for the ground state and thus we have no knowledge of the properties at finite temperatures. 

As seen from table 7.1, the enthalpy of formation of this compound far exceeds that of the parent bulk compound. In this case we have no experimental value to measure our results to, but note that the individual energies correspond well with materials project, despite of potential differences in computational settings and methods. 
