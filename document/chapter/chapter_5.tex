...
However several studies have been conducted displaying the limitations of PBE, in which both LDA and PBE severly underestimate the band gap compared to experimental data and results of hybrid-functionals. Examples are \cite{lda_pbe_fails}, where the hybrid functional B3PW91 and experimental meassures of \{CuInSe2} result in a band gap around 1 eV, but LDA and GGA find a gap in the range 0.0 - 0.01 eV. Moreover the LDA and GGA functional fail to locate a band gap in \ch{La2CuO4} that experimentally have a band gap of 2 eV \cite{lda_pbe_limitations}. This is escpecially troublesome for studies of for example narrow gap thermoelectrics and photovoltaics where accurate values of the band gap and states at the band edges is critical.   

Further, we can extended this idea to include the Laplacian $\nabla^2$ of the density. This range of methods is called meta-GGA, in this project we use a meta-GGA functional called \textit{Strongly Constrained Appropriately Normed}, or SCAN \cite{scan}. \textbf{Write a brief discussion of these, see for example \cite{scan2}}. SCAN outperfomrs PBE \cite{scan_pbe}. \cite{scan_hse06}
...
We also tried MBJ but failed to converge calculations, a shame considering this is one of the highest rated meta-GGA and XC-functionals in general of the band gap of solids. 
