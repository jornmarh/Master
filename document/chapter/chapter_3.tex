...
With this approach, \cite{sqsfull} managed by mimicking the correlation functions exact for the first two shells, to reduce the computational measures of an accurate models. In this exact study they matched the results of an $N \rightarrow \infty$ by an $N=8$ SQS. In the final section of this chapter, we will take a look at the recent advances in the SQS method and application for high-entropy alloys. 
 
\textbf{Write a final conclusion on this SQS derivation and motivate the incredibly bored reader for the coming sections. Maybe add a figure somewhere?} 

\section{Application of SQS to high-entropy alloys}
The success of the SQS-approach is related to to the fact that we create simple periodic structures that allow for standard DFT based methods to calculate properties such as the total energy, charge density and electronic band structure \cite{sqs_dos}, \cite{sqs_bg}. However the application to high-entropy alloys have been limited prior to the last decade due to the increased computational efforts demanded in modeling of disordered multi-component structures. \cite{hea2016_ch10} in 2016, presented a comprehensive review of the application of SQS to high-entropy alloys based on ab initio simulations in the framework of DFT and VASP. The key takeaway from this study was that that the sensitivity to the size of the SQS supercells limited both the broadness of the study, and the accuracy of the results. For example the ground state structure was wrongly predicted for a $CoCrFeMnNi$ alloy with less than 24 total atoms, but correctly predicted FCC for strictures containing more than 64 atoms. This dependence on cell size is dominantly displayed when considering the pair-distribution functions (PDF) of various high-entropy alloys. M.C Gao et al. found that particularly the PDFs of Cr-Mn improved drastically from a 20-atom to 125 and 250 cell in a $CoCrFeMnNi$ SQS.        


\textbf{Write a conclusion on the improvement with mc-SQS and recent use of SQS on HEA, (cite recent articles on SQS HEA). And conclude this section with a concern over the computational aspects vs accuracy and the consequence for finding the optimal parameters, such as magnetic configurations.}       
